...
public class LoginController {
	...
	if(null != username){
		...
		if(null != buyer){
			...
			//5:保存用户名到Session中 (redis中)用户对象到Session
			sessionProvider.setAttribuerForUsername(RequestUtils.getCESSIONID(request,response), buyer.getUsername());
			//6:跳转到之前访问页面
  ...
  //判断用户石佛登陆
    /*
     * json 跨域 
     * @ResponseBody  返回值不会解析成跳转 而是解析成JSON
     * MappingJacksonValue  返回东西就是data  
     */
