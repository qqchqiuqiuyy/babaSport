...
名称: <input type="text" name="name" value="${name }"/>
	...
	<select name="isShow">
		...
		<option selected="selected" value="0" <c:if test="${!isShow }">selected="selected"</c:if>   >下架</option>
	</select>
...
<table cellspacing="1" cellpadding="0" width="100%" border="0" class="pn-ltable">
	...
	<tbody class="pn-ltbody">
		<c:forEach items="${pagination.list }" var="product">
			...
			<td align="center"><img width="50" height="50" src="${product.imges[0]}"/></td>
			...
			<td align="center">
			<a href="#" class="pn-opt">查看</a> | <a href="/brand/toEdit.do?id=${product.id}" class="pn-opt">修改</a> | <a href="#" onclick="if(!confirm('您确定删除吗？')) {return false;}" class="pn-opt">删除</a> | <a href="/sku/list.do?productId=${product.id  }" class="pn-opt">库存</a>
			</td>
		</tr>
